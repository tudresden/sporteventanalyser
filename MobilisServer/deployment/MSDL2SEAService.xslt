...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="packageNamespace"
		select="'de.tudresden.inf.rn.mobilis.server.services.sea.service.proxy'" />
	...
	<xsl:template match="/">
		...
		<xsl:apply-templates select="/"
			mode="generateIIncomingInterface" />
		<xsl:value-of select="$newline" />
		<xsl:apply-templates select="/"
			mode="generateIOutgoingInterface" />
		<xsl:value-of select="$newline" />

		<!-- creates callback interface -->
		<xsl:apply-templates select="/"
			mode="generateXMPPCallbackInterface" />
		<xsl:value-of select="$newline" />
		...
		<xsl:apply-templates
			select="/msdl:description/msdl:types/xs:schema/xs:complexType" mode="generateTypeInfoClass" />
	...
	<xsl:template match="/" mode="generateProxyClass">
		...
		<xsl:result-document href="{$fileName}">
			...
			<xsl:value-of select="$IOutgoingInterfaceName" />
			<xsl:text> _bindingStub;</xsl:text>
			...
			<xsl:text>public </xsl:text>
			<xsl:value-of select="$IOutgoingInterfaceName" />
			<xsl:text> getBindingStub(){</xsl:text>
			<xsl:value-of select="$newline" />
			<xsl:value-of select="$indent" />
			<xsl:value-of select="$indent" />
			...
			<xsl:value-of select="$newline" />
			<xsl:apply-templates
				select="/msdl:description/msdl:interface/msdl:operation[@pattern=$mepOutOnly or @pattern=$mepOutIn or @pattern=$mepInOut]"
				mode="generateProxyMethod" />
			<xsl:value-of select="$newline" />
	...
	<xsl:template match="/msdl:description/msdl:interface/msdl:operation"
		...
		<xsl:variable name="outClassName"
		...
		<xsl:text>( String toJid</xsl:text>
		...
		<xsl:if
			test="count(/msdl:description/msdl:types/xs:schema/xs:element[@name=$outClassName]/xs:complexType/xs:sequence/xs:element) &gt; 0">
			<xsl:text>, </xsl:text>

			<xsl:apply-templates
				select="/msdl:description/msdl:types/xs:schema/xs:element[@name=$outClassName]/xs:complexType/xs:sequence"
				mode="generateParameterlistOfElement">
				<xsl:with-param name="withTypes">
					1
				</xsl:with-param>
			</xsl:apply-templates>
		</xsl:if>
		...
		<xsl:if test="@pattern=$mepOutIn">
			...
			<xsl:variable name="inClassName"
				select="substring-after(msdl:input/@element,':')" />
			<!-- If "out"-Element exists, create callback parameter with type; element 
				attribute cannot be empty -->
			<xsl:text>, </xsl:text>
			<xsl:value-of select="$xmppCallbackInterfaceName" />
			<xsl:text>&lt; </xsl:text>
		...
		<xsl:value-of select="$outClassName" />
		<xsl:text> out = new </xsl:text>
		<xsl:value-of select="$outClassName" />
		<xsl:text>( </xsl:text>
		<xsl:apply-templates
			select="/msdl:description/msdl:types/xs:schema/xs:element[@name=$outClassName]/xs:complexType/xs:sequence"
	...
	<xsl:template match="xs:element" mode="parseElementDatatype">
		...
		<xsl:choose>
			...
			<xsl:when
				test="compare(./@maxOccurs, 'unbounded') = 0 and number($asSimple) != 1">
				...
				<xsl:choose>
					...
					<xsl:when test="compare($elementType,'xs:int') = 0">
						<xsl:text>Integer</xsl:text>
					</xsl:when>
					<xsl:when test="compare($elementType,'xs:long') = 0">
						<xsl:text>Long</xsl:text>
					</xsl:when>
					<xsl:when test="compare($elementType,'xs:boolean') = 0">
						<xsl:text>Boolean</xsl:text>
					</xsl:when>
					<xsl:when test="compare($elementType,'xs:double') = 0">
						<xsl:text>Double</xsl:text>
					</xsl:when>
					<xsl:when test="compare($elementType,'xs:float') = 0">
						<xsl:text>Float</xsl:text>
					</xsl:when>
					<xsl:when test="compare($elementType,'xs:byte') = 0">
						<xsl:text>Byte</xsl:text>
					</xsl:when>
					<xsl:when test="compare($elementType,'xs:short') = 0">
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:when
						test="contains($elementType,'tns:') or compare($elementType,'xs:int') = 0 or compare($elementType,'xs:long') = 0 or compare($elementType,'xs:boolean') = 0 or compare($elementType,'xs:double') = 0 or compare($elementType,'xs:float') = 0 or compare($elementType,'xs:byte') = 0 or compare($elementType,'xs:short') = 0">
						<xsl:value-of select="substring-after($elementType,':')" />
					</xsl:when>
	...
	<xsl:template match="/" mode="generateIIncomingInterface">
		<xsl:variable name="fileName"
			select="concat($IIncomingInterfaceName,'.java')" />
		<xsl:value-of select="$fileName" />
		<xsl:result-document href="{$fileName}">
			<xsl:if test="string-length($packageNamespace) > 0">
				<xsl:text>package </xsl:text>
			...
			<xsl:value-of select="$IIncomingInterfaceName" />
			<xsl:text> {</xsl:text>
			<xsl:value-of select="$newline" />
			...
			<xsl:for-each
				select="msdl:description/msdl:interface/msdl:operation[@pattern=$mepInOnly or @pattern=$mepOutIn or @pattern=$mepInOut]">
				...
				<!-- If pattern is in-out an unknown XMPPError can be reponded (e.g. 
					service unavailable) -->
				<xsl:if test="@pattern = $mepOutIn">
					<xsl:value-of select="$newline" />
					<xsl:value-of select="$indent" />

					<xsl:text>void on</xsl:text>
					<xsl:value-of select="$methodName" />
					<xsl:text>Error</xsl:text>
					<xsl:text>( </xsl:text>
					<xsl:value-of select="substring-after(msdl:output/@element,':')" />
					<xsl:text> in);</xsl:text>
					<xsl:value-of select="$newline" />
				</xsl:if>
	...
	<xsl:template match="/" mode="generateIOutgoingInterface">
		<xsl:variable name="fileName"
			select="concat($IOutgoingInterfaceName,'.java')" />
		<xsl:value-of select="$fileName" />
		<xsl:result-document href="{$fileName}">
			<xsl:if test="string-length($packageNamespace) > 0">
				<xsl:text>package </xsl:text>
	...
	<xsl:template match="/" mode="generateXMPPCallbackInterface">
		<xsl:variable name="fileName"
			select="concat($xmppCallbackInterfaceName,'.java')" />
		<xsl:value-of select="$fileName" />
		<xsl:result-document href="{$fileName}">
			<xsl:if test="string-length($packageNamespace) > 0">
				<xsl:text>package </xsl:text>
	...
	<xsl:template match="xs:complexType" mode="generateTypeInfoClass">
		<xsl:variable name="namespace" select="./@xmpp:ident" /><!-- 
			serviceNs + '#type:' + classname -->
		<xsl:variable name="typeInfoClassName" select="./@name" />

		<!-- creates output file -->
		<xsl:variable name="fileName" select="concat($typeInfoClassName,'.java')" />
		<xsl:value-of select="$fileName" />
		<xsl:result-document href="{$fileName}">
			...
			<xsl:text>public class </xsl:text>
			<xsl:value-of select="$typeInfoClassName" />
			<xsl:text> implements XMPPInfo {</xsl:text>
			<xsl:value-of select="$newline" />
			<xsl:value-of select="$newline" />
			...
			<xsl:if test="count(./xs:sequence/xs:element) > 0">
				...
				<xsl:apply-templates select="./xs:sequence/xs:element"
					mode="generateAndAssignClassProperty" />
				<xsl:value-of select="$newline" />
				<xsl:value-of select="$newline" />

				<!-- Generate Full constructor -->
				<xsl:apply-templates select="./xs:sequence"
					mode="generateFullConstructor">
			...
			<xsl:apply-templates select="./xs:sequence"
				mode="generateFunctionFromXML" />
			<xsl:value-of select="$newline" />
			<xsl:value-of select="$newline" />

			<xsl:apply-templates select="/"
				mode="generateFunctionGetChildElement">
				<xsl:with-param name="childElement">
					<xsl:value-of select="$typeInfoClassName" />
				</xsl:with-param>
			</xsl:apply-templates>
			<xsl:value-of select="$newline" />
			...
			<xsl:apply-templates select="/"
				mode="generateFunctionGetNamespace">
			...
			<xsl:apply-templates select="./xs:sequence"
				mode="generateFunctionPayloadToXML" />
			<xsl:value-of select="$newline" />
			<xsl:value-of select="$newline" />
			<xsl:value-of select="$newline" />
	...
	<xsl:template match="/msdl:description/msdl:binding/msdl:operation"
		...
		<xsl:for-each
			select="/msdl:description/msdl:interface/msdl:operation[@name=$interfaceOperationName]/*">
			...
			<xsl:choose>
				<xsl:when test="$elementName">
					...
					<xsl:variable name="xmppType"
						select="/msdl:description/msdl:binding/msdl:operation[@ref = $bindingOperationName]/*[name() = $operationDirection]/@xmpp:type" />
					<xsl:variable name="fileName"
						select="concat($typeBeanClassName,'.java')" />
					...
					<xsl:result-document href="{$fileName}">
						...
						<xsl:text>public class </xsl:text>
						<xsl:value-of select="$typeBeanClassName" />
						...
						<xsl:value-of select="$newline" />
						<xsl:if
							test="count(/msdl:description/msdl:types/xs:schema/xs:element[@name=$typeBeanClassName]/xs:complexType/xs:sequence/xs:element) > 0">
							<!-- Generate and assign class properties -->
							<xsl:apply-templates
								select="/msdl:description/msdl:types/xs:schema/xs:element[@name=$typeBeanClassName]/xs:complexType/xs:sequence/xs:element"
								mode="generateAndAssignClassProperty" />
							<xsl:value-of select="$newline" />
							<xsl:value-of select="$newline" />
							...
							<!-- Generate Full constructor -->
							<xsl:apply-templates
								select="/msdl:description/msdl:types/xs:schema/xs:element[@name=$typeBeanClassName]/xs:complexType/xs:sequence"
								mode="generateFullConstructor">
						...
						<xsl:apply-templates select="/"
							mode="parseXmppTypeToBeanType">
							<xsl:with-param name="xmppType">
						...
						<xsl:if
							test="count(/msdl:description/msdl:types/xs:schema/xs:element[@name=$typeBeanClassName]/xs:complexType/xs:sequence) = 0">
							<xsl:text>	@Override
	public void fromXML( XmlPullParser parser ) throws Exception {}</xsl:text>
						...
						<xsl:apply-templates
							select="/msdl:description/msdl:interface/msdl:operation/*[@element=$elementName]"
							mode="generateFunctionClone" />
						<xsl:value-of select="$newline" />
						<xsl:value-of select="$newline" />
	...
	public String payloadToXML() { return ""; }</xsl:text>
						...
						<xsl:if
							test="../@pattern = $mepInOut and compare(name(),'msdl:input') = 0">
							<xsl:apply-templates select=".."
								mode="generateFaultFunctions">
	...
	<xsl:template match="/" mode="parseXmppTypeToBeanType">
		...
		<xsl:choose>
			<xsl:when test="compare(lower-case($xmppType),'set') = 0">
				<xsl:text>XMPPBean.TYPE_SET</xsl:text>
			</xsl:when>
			<xsl:when test="compare(lower-case($xmppType),'get') = 0">
				<xsl:text>XMPPBean.TYPE_GET</xsl:text>
			</xsl:when>
			<xsl:when test="compare(lower-case($xmppType),'result') = 0">
				<xsl:text>XMPPBean.TYPE_RESULT</xsl:text>
			</xsl:when>
			<xsl:when test="compare(lower-case($xmppType),'chat') = 0">
				<xsl:text>XMPPBean.TYPE_GET</xsl:text>
			</xsl:when><!-- TODO: muss in chat umgewandelt werden, wenn xmppbean neu 
				ist -->
			<xsl:when test="compare(lower-case($xmppType),'error') = 0">
				<xsl:text>XMPPBean.TYPE_ERROR</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				XMPPBean.TYPE_GET
			</xsl:otherwise>
	...
	<xsl:template match="xs:element" mode="generateAndAssignClassProperty">
		...
		<xsl:text> = </xsl:text>
		<xsl:apply-templates select="."
			mode="generateElementDatatypeInitializer" />
		<xsl:text>;</xsl:text>
	...
	<xsl:template match="xs:sequence" mode="generateFullConstructor">
		...
		<xsl:value-of select="$indent" />
		<xsl:text>public </xsl:text>
		<xsl:value-of select="$typeClassName" />
		<xsl:text>( </xsl:text>
		<xsl:apply-templates select="."
			mode="generateParameterlistOfElement">
			<xsl:with-param name="withTypes">
		...
		<xsl:apply-templates select="./xs:element"
			mode="generateParameterAssignment" />
	...
	<!-- generate the XMPPBean function fromXML which is used to parse a XMPP 
		message to XMPPBean -->
	<xsl:template match="xs:complexType/xs:sequence" mode="generateFunctionFromXML">
				...
				<xsl:when test="compare(./@maxOccurs, 'unbounded') = 0">
					<xsl:choose>
						...
						<xsl:when test="contains(@type,'xs:string')">
							<xsl:value-of select="@name" />
							<xsl:text>.add( parser.nextText() );</xsl:text>
						</xsl:when>
						...
						<xsl:otherwise>
							...
							<xsl:apply-templates select="."
								mode="generateDatatypeParsePrefix" />
		...
		<!-- if this function is generated for a XMPPBean, parse errors too. For 
			XMPPInfo it's not necessary -->
	...
	<!-- parsing datatypes beside string requires special parsing funcitons 
		like for an integer:= Integer.parseInt( str ) -->
	<xsl:template match="xs:element" mode="generateDatatypeParsePrefix">
		...
		<xsl:choose>
			...
			<xsl:when test="compare($elementType,'xs:long') = 0">
				<xsl:text>Long.parseLong</xsl:text>
			</xsl:when>
			<xsl:when test="compare($elementType,'xs:boolean') = 0">
				<xsl:text>Boolean.parseBoolean</xsl:text>
			</xsl:when>
			<xsl:when test="compare($elementType,'xs:double') = 0">
				<xsl:text>Double.parseDouble</xsl:text>
			</xsl:when>
			<xsl:when test="compare($elementType,'xs:float') = 0">
				<xsl:text>Float.parseFloat</xsl:text>
			</xsl:when>
			<xsl:when test="compare($elementType,'xs:byte') = 0">
				<xsl:text>Byte.parseByte</xsl:text>
			</xsl:when>
			<xsl:when test="compare($elementType,'xs:short') = 0">
				<xsl:text>Short.parseShort</xsl:text>
			</xsl:when>
	...
	<xsl:template match="/msdl:description/msdl:interface/msdl:operation/*"
		...
		<xsl:value-of select="$indent" />
		<xsl:value-of select="$className" />
		<xsl:text> clone = new </xsl:text>
		<xsl:value-of select="$className" />
		<xsl:text>( </xsl:text>
		<xsl:apply-templates
			select="/msdl:description/msdl:types/xs:schema/xs:element[@name=$className]/xs:complexType/xs:sequence"
			mode="generateParameterlistOfElement" />
		<xsl:text> );</xsl:text>
	...
	<xsl:template match="xs:sequence" mode="generateFunctionPayloadToXML">
		...
		<xsl:choose>
			<xsl:when test="number($withError) = 1">
				<xsl:text>payloadT</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>t</xsl:text>
			</xsl:otherwise>
	...
	<!-- generates a xml tag of a XMPPBean parameter regarding the type of property 
		(simple, list, XMPPInfo) -->
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					...
					<xsl:when test="contains(./@type,'tns:')">
						<xsl:text>" + this.</xsl:text>
						<xsl:value-of select="./@name" />
						<xsl:text>.getChildElement() + "</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="./@name" />
					</xsl:otherwise>
	...
	<xsl:template match="msdl:interface/msdl:operation" mode="generateFaultFunctions">
		...
		<xsl:for-each select="./msdl:outfault">
			...
			<xsl:apply-templates
				select="/msdl:description/msdl:binding/msdl:fault[@ref = $faultRef]"
				mode="generateFunctionBuildFault">
	...
	<!-- generates a special fault function to simplify the usage of creating 
		a predifend error message -->
	<xsl:template match="msdl:binding/msdl:fault" mode="generateFunctionBuildFault">
		...
		<xsl:text>(String detailedErrorText){</xsl:text>
		...
		fault.setType( </xsl:text>
		<xsl:value-of select="$xmppBeanClassName" />
		<xsl:text>.TYPE_ERROR );</xsl:text>
		<xsl:value-of select="$newline" />
		<xsl:value-of select="$indent" />
		<xsl:value-of select="$indent" />

		<xsl:text>fault.errorType = "</xsl:text>
		<xsl:value-of select="@xmpp:errortype" />
		<xsl:text>";</xsl:text>
		<xsl:value-of select="$newline" />
		<xsl:value-of select="$indent" />
		<xsl:value-of select="$indent" />

		<xsl:text>fault.errorCondition = "</xsl:text>
		<xsl:value-of select="@xmpp:errorcondition" />
		<xsl:text>";</xsl:text>
		<xsl:value-of select="$newline" />
		<xsl:value-of select="$indent" />
		<xsl:value-of select="$indent" />

		<xsl:text>fault.errorText = "</xsl:text>
		<xsl:value-of select="@xmpp:errortext" />
		<xsl:text>";</xsl:text>
		<xsl:value-of select="$newline" />
		<xsl:value-of select="$newline" />
		<xsl:value-of select="$indent" />
